...
/// </summary>
public sealed class BertTransformer : IVectorizer<DenseVector>, IDisposable
{
    /// <summary>
    ...
    public BertTransformer(string? tokenizerJsonPath = null, string? onnxModelPath = null, BertTransformerSettings? settings = null)
    {
    ...
    public ReadOnlyCollection<DenseVector> Transform(IEnumerable<string> documents)
    {
        ...
        using var output = OrtValue.CreateTensorValueFromMemory<float>(OrtMemoryInfo.DefaultInstance, _outputBuffer, [
            batchSize,
            outputDimension
        ]);
        int documentCount = 0;
        List<DenseVector> embeddings =
        [
        ];
        foreach (var batch in Tokenizer.CreateBatchEnumerator(sourceEnumerable: documents.Select(doc => (documentCount++, doc)), tokensPerInput: _settings.InputDimension, batchSize: batchSize, stride: _settings.StrideSize))
        {
            var inputIds = batch.InputIds;
            var attentionMask = batch.AttentionMask;
            using var inputIdTensor = OrtValue.CreateTensorValueFromMemory(OrtMemoryInfo.DefaultInstance, inputIds, [
                batchSize,
                inputDimension
            ]);
            using var attentionMaskTensor = OrtValue.CreateTensorValueFromMemory(OrtMemoryInfo.DefaultInstance, attentionMask, [
                batchSize,
                inputDimension
            ]);

            // Inference
            _session.Run(runOptions, inputNames: inputLayerNames, inputValues:
                [
                    inputIdTensor,
                    attentionMaskTensor
                ], outputNames: outputLayerNames, outputValues:
                [
                    output
                ]);
    ...
    /// <param name="documents">A sequence of document strings to be vectorized.</param>
    /// <returns>A sequence of document vectors as float arrays.</returns>
    public ReadOnlyCollection<DenseVector> FitThenTransform(IEnumerable<string> documents) => Transform(documents);
}
